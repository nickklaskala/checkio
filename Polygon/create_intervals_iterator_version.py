...
# From a set of ints (which will be an iterator of the sorted list) you have to create a list of closed intervals as tuples,
# #so the intervals are covering all the values found in the set. After that you have to create an iterator object which is linked to this list.
#
...
def create_intervals(data):
	...
	for i in data:
		myList.append(i)
	l=sorted([x for x in myList if x-1 not in myList])
	r=sorted(x for x in myList if x+1 not in myList)
	rst=iter(list(zip(l,r)))
	return rst
...
if __name__ == '__main__':
    ...
    assert hasattr(res, '__next__'), "your function should return the next object"
