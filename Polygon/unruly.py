#!/usr/bin/env checkio --domain=py run unruly

# This mission is an adaptation of the "Unruly" game (fromSimon Tatham's Portable Puzzle Collection).    If you are lost or just want to play, the game is availablehere.
# 
# You have an uncomplete rectangular grid    and have to colour all the cells, either black or white,    with two constraints:No three consecutives cells, horizontally or vertically, are all black or all white.Each row and column contains as many black cells as white cells.Obviously, you can only change empty cells, not colored cells.
# 
...
# 
# Preconditions:Given puzzles are solvable.You can solve all puzzles without making any guesses.6 ≤ len(grid) ≤ 40 and 6 ≤ len(grid[0]) ≤ 30.all(len(row) == len(grid[0]) for row in grid).
# 
# 
# END_DESC
