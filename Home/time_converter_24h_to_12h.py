...
def time_converter(time):
    h=str(int(time[:2]) if int(time[:2])<=12 else int(time[:2])%12)
    h=h if time!='00:00' else '12'
    t=(h+time[2:]+ (' a.m.' if int(time[:2])<12 else ' p.m.'))
    print(t)
    return t
...
if __name__ == '__main__':
    ...
    assert time_converter('07:00') == '7:00 a.m.'
