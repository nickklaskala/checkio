...
def checkio(cakes):
	colinear = lambda x, y, z:(y[0]-x[0])*(z[1]-x[1]) == (y[1]-x[1])*(z[0]-x[0])
	rows = []

	for a, z in combinations(cakes, 2):
		line = set(tuple(p) for p in cakes if colinear(a, p, z))
		if len(line) > 2 and line not in rows:
			rows.append(line)
	return len(rows)
