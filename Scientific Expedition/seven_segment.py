...
def seven_segment(lit_seg, broken_seg):
	...
	lw,up,m=[],[],[]
	for num,nset in upNumMap.items():
		if set(up_lit_seg).issubset(nset) and set(nset).issubset(up_tot_seg):
			up.append(num)
	for num,nset in lwNumMap.items():
		if set(lw_lit_seg).issubset(nset) and set(nset).issubset(lw_tot_seg):
			lw.append(num)
