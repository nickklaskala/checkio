...
declare @siteid int=(select siteid from site where name='SITENAME')
...
create table #UserRoles(
	...
	Department varchar(100) collate Latin1_General_CI_AS,
	Department2 varchar(100) collate Latin1_General_CI_AS,
	Department3 varchar(100) collate Latin1_General_CI_AS,
	Department4 varchar(100) collate Latin1_General_CI_AS,
	 )
...
while @count1<=(select count(*) from #Temp)
	...
	left join systemrole as c on c.name='''+@SystemRole+'''
	left join Department as d on iif(a.Department=''all'',a.Department, d.name)=a.Department and d.siteid='+@siteid+'
	left join PersonSystemRole as x on x.personid=b.personid and x.systemroleid=c.systemroleid and x.Departmentid=r.Departmentid
	where a.Department<>'''' and x.personsystemroleid is null and a.['+@SystemRole+']=''y'''

	exec (@sqlcommand)

	set @sqlcommand=replace(@sqlcommand,'a.Department','a.Department2')
	exec (@sqlcommand)

	set @sqlcommand=replace(@sqlcommand,'a.Department2','a.Department3')
	exec (@sqlcommand)

	set @sqlcommand=replace(@sqlcommand,'a.Department3','a.Department4')
	exec (@sqlcommand)
