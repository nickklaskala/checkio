...
;with t1 as (
	...
	select
		*,
		...
		)/5 as decimal)as FiveDayNewconfirmedAvg
	from t2
), t4 as (
	select
		*,
		cast(cast(iif(FiveDayNewconfirmedAvg=0,1,FiveDayNewconfirmedAvg)  as numeric)/lag(iif(FiveDayNewconfirmedAvg=0,1,FiveDayNewconfirmedAvg),1,1) over(partition by state order by DateRecorded)as decimal(4,3)) as fivedayPercentChangeInNewConfirmedAvg
	from t3
)
select
	*,
	cast(FiveDayNewconfirmedAvg*FiveDayPercentChangeInNewconfirmedAvg as numeric) as'EstNextDayNewConfirmed'
from t4
where 1=1
--and state='ca'
and DateRecorded=(select max(daterecorded) from #cases)
--order by DateRecorded
order by FiveDayPercentChangeInNewconfirmedAvg desc
...
;with t1 as (
	...
	select
		*,
		cast(cast(iif(FiveDayNewconfirmedAvg=0,1,FiveDayNewconfirmedAvg)  as numeric)/lag(iif(FiveDayNewconfirmedAvg=0,1,FiveDayNewconfirmedAvg),1,1) over(partition by state order by DateRecorded)as decimal(10,3)) as fivedayPercentChangeInNewConfirmedAvg
	from t3
)
select
	*,
